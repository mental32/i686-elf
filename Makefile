...
gdb: $(BUILD_DIR) $(LIB_DIR) $(BUILD_DIR)/gdb-$(GDB_VERSION).tar.gz
	...
	cd $(GDB_BUILD_DIR)/obj\
		...
		&& make -j 4 tooldir=$(GDB_BUILD_DIR)/obj\
		&& make -j 4 tooldir=$(GDB_BUILD_DIR)/obj install\
...
gcc: $(BUILD_DIR) $(LIB_DIR) $(BUILD_DIR)/gcc-$(GCC_VERSION).tar.gz
	...
	cd $(GCC_BUILD_DIR)/obj\
		...
		&& make -j 4 tooldir=$(GCC_BUILD_DIR)/obj all-gcc\
		&& make -j 4 tooldir=$(GCC_BUILD_DIR)/obj install-gcc\
